...
const transformAndSortEntries = createSelector(
  ...
  (response) => {
    ...
    const entriesArray = Object.keys(response)
      .map((key) => ({
        id: key,
        ...response[key],
      }))
...
const selectEntriesResult = (userId) =>
  journalApi.endpoints.getEntries.select(userId);
const selectCategoriesResult = journalApi.endpoints.getCategories.select();

// ENTRIES ENRICHED WITH ASSOCIATED CATEGORY
export const selectEnrichedEntries = (userId) =>
  createSelector(
    ...
    (entriesResult, categoriesResult) => {
      ...
      return entries.map((entry) => ({
        ...entry,
        category: categories.find((cat) => cat.id === entry.categoryId) || null,
      }));
    }
  );

export const selectEnrichedEntryById = createSelector(
  (state, { userId, entryId }) =>
    journalApi.endpoints.getEntryById.select({ userId, entryId })(state)?.data,
  ...
  (entryData, categoriesData, entryId) => {
    ...
    const cleanCategories = categoriesData.filter(Boolean);

    return {
      id: entryId,
      ...entryData,
      category:
        cleanCategories.find((cat) => cat.id === entryData.categoryId) || null,
    };
  }
);
